...
/**
 * SVG sequence diagram showing the Embedded Form (Embedded Checkout) flow.
...
const STEPS: Step[] = [
  { from: 0, to: 1, label: "Open /embedded-form" },
  { from: 1, to: 2, label: "POST create-embedded-checkout-session" },
  { from: 2, to: 3, label: "sessions.create (embedded, line_items)" },
  { from: 3, to: 2, label: "client_secret", dashed: true },
  { from: 2, to: 1, label: "clientSecret", dashed: true },
  {
    from: 1,
    to: 1,
    label: "EmbeddedCheckout mounts with fetchClientSecret",
    self: true,
  },
  { from: 0, to: 1, label: "Complete payment in Stripe iframe" },
...
export function EmbeddedFormFlowDiagram() {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Embedded Form checkout flow sequence diagram"
    >
      <defs>
        <marker
          id="ef-arrow"
      ...
      {STEPS.map((step, i) => {
        ...
        const marker = step.dashed ? "url(#ef-arrow-d)" : "url(#ef-arrow)";
