...
/**
 * SVG sequence diagram showing the Payment Element checkout flow.
...
const STEPS: Step[] = [
  { from: 0, to: 1, label: "Open /payment-element" },
  ...
  { from: 2, to: 1, label: "clientSecret", dashed: true },
  {
    from: 1,
    to: 1,
    label: "CheckoutProvider + PaymentElement renders",
    self: true,
  },
  { from: 0, to: 1, label: "Enter payment details, click Pay" },
  { from: 1, to: 3, label: "checkout.confirm()" },
...
export function PaymentElementFlowDiagram() {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Payment Element checkout flow sequence diagram"
    >
      <defs>
        <marker
          id="pe-arrow"
      ...
      {STEPS.map((step, i) => {
        ...
        const marker = step.dashed ? "url(#pe-arrow-d)" : "url(#pe-arrow)";
